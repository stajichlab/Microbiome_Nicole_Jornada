...
```{r}
summary(LAC.Crust_type.SHP, p = 0.05, type = "short")
```
...
```{r}
gr <- LAC.Crust_type.SHP$maxcls[LAC.Crust_type.SHP$pval<=0.05]
iv <- LAC.Crust_type.SHP$indcls[LAC.Crust_type.SHP$pval<=0.05]
pv <- LAC.Crust_type.SHP$pval[LAC.Crust_type.SHP$pval<=0.05]
fr <- apply(ASV.SHP.no0[,-1]>0, 2, sum)[LAC.Crust_type.SHP$pval<=0.05]
