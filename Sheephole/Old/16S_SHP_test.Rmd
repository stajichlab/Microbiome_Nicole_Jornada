...
```{r }
plot_ordination(Bac.physeq.prune.rarefy, physeq.prune.ord, type = "samples", color = "Crust_type", shape = "Crust_type", label = "SampleName")  + theme_bw() + ggtitle("Bacterial Beta Diversity (PCoA) by Crust_type (Sheephole)") + theme(plot.title = element_text(hjust = 0.5)) + stat_ellipse(geom = "polygon", alpha = 1/8, aes(fill = Crust_type)) + geom_point(size=3)
```

```{r}
pdf("./Figures/DOE.Beta_div_Bac_SHP_w_SampleName.pdf", width = 8, height = 5)
plot_ordination(Bac.physeq.prune.rarefy, physeq.prune.ord, type = "samples", color = "Crust_type", shape = "Crust_type", label = "SampleName")  + theme_bw() + ggtitle("Bacterial Beta Diversity (PCoA) by Crust_type (Sheephole)") + theme(plot.title = element_text(hjust = 0.5)) + stat_ellipse(geom = "polygon", alpha = 1/8, aes(fill = Crust_type)) + geom_point(size=3)
dev.off()
```
