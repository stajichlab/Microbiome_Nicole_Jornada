---
title: "16S_SHP_test"
author: "Nat Pombubpa"
date: "Updated on May 13, 2019"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

#Documentation for Qiime2 Silva extracted length classifier on Sheephole crusts samples
This is a documentation for otu table, taxonomy table, and tree which were generated from Qiime2 using DADA2 and Silva database classifier extracted lenght (515-806). 

###STEP1: Load all necessary packages for analysis
More information about Phyloseq can be found at the following link: [Phyloseq](https://joey711.github.io/phyloseq/)
If you get error in this step, you probably need to install any packages which causes error.

```{r warning=FALSE, message=FALSE}
library(ape)
library(vegan)
library(plyr)
library(dplyr)
library(scales)
library(grid)
library(reshape2)
library(phyloseq)
library(magrittr)
library(ggplot2)
library(ggpubr)
library(data.table)
library(tidyr)
#library(labdsv)
library(indicspecies)
```

###STEP2: Transition data from Qiime2 to Phyloseq
Note: otu table and taxonomy table have been fixed right after Qiime2. Nat's Qiime2 script already include this step.
However, you can simply fix by changing "#Feature ID" to "OTU ID" or "OTU.ID" and the following commnand should work fine.

####STEP2.1: Import otu table from Qiime2

```{r}
otu = read.table(file="MojaveBacQ2FWLE.otu_table.fix.txt", header=T, sep='\t')
```

```{r}
head(otu)
```

Please note dimension of otu table. For this project, there are 21697 rows(OTU.ID) which will need to be matched to taxonomy table before starting Phyloseq analysis.
```{r}
dim(otu)
```

####STEP2.2: Import taxonomy table from Qiime2

```{r}
tax <- read.table(file="taxonomy.fix.tsv", sep='\t', header=TRUE)
head(tax)
```

Taxonomy table contains 22081 rows(OTU.ID) which is more than otu table. (Mitochondria and Chloroplast were removed in otu table which resulted in less number of OTU.ID in otu table.)
```{r}
dim(tax)
```

####STEP2.3: Match OTU.ID between otu and taxonomy table
Keep only OTU.ID in taxonomy table  that were found in otu table.
```{r}
tax_filtered <- tax[tax$OTU.ID %in% otu$OTU.ID,]
head(tax_filtered)
```

Check dimension of tax_filterd table: the number of OTU.ID should be the same as OTU table (in this case: 21697 OTU.ID)
```{r}
dim(tax_filtered)
```

Number of rows for "tax_filtered" and "otu" should be the same.
```{r}
dim(otu)
```

####STEP2.4: Select only OTU.ID column and taxonomy for taxonomy table
Taxonomy table from Qiime2 also contains the third column which is "Confidence". We will remove this column before loading data to Phyloseq.
```{r}
tax_filtered = tax_filtered[,c(1,2)]
```

After selection, taxonomy column will be separated into 7 levels as "Kingdom","Phylum","Class","Order", "Family", "Genus","Species"
```{r}
tax_filtered = separate(tax_filtered, Taxon, c("Kingdom","Phylum","Class","Order", "Family", "Genus","Species"), sep= ";", remove=TRUE)
```

At the end of this step, taxonomy table will contain OTU.ID column + 7 levels taxonomy columns (total of 8 columns). 
```{r}
dim(tax_filtered)
```

```{r}
head(tax_filtered)
```

Save "filtered" taxonomy table for Phyloseq and future usage.
```{r}
write.csv(tax_filtered, file="taxonomy_phyloseq_fix.csv")
```

###STEP3: Import otu table for analysis using Phyloseq 

```{r warning=FALSE}
otus <- read.table("MojaveBacQ2FWLE.otu_table.fix.txt",
                   header=T,sep="\t",row.names = 1)
otumat <- as(as.matrix(otus), "matrix")
OTU = otu_table(otumat, taxa_are_rows = TRUE)
```

###STEP4: Import taxonomy table for analysis using Phyloseq
This taxonomy table was created from STEP2.
```{r warning=FALSE}
taxmat <- read.csv("taxonomy_phyloseq_fix.csv", 
                   sep=",",row.names=1)
row.names(taxmat) = taxmat$OTU.ID
taxmat <- as(as.matrix(taxmat),"matrix")
TAX = tax_table(taxmat)
```

###STEP5: Import tree file for analysis using Phyloseq

```{r warning=FALSE}
treefile = "treeWMC.nwk"
tree = read.tree(treefile)
```

###STEP6: Import mapping file for analysis using Phyloseq

1.Check mapping file before import to R, R will automatically change sample name that starts with number or contain “-” in sample name. If you get error in this step, you should check sample name first. 

2.First column of first row should not start with #, R will not read the first row that starts with #

3. You can choose which samples to include in analysis by indicating specific group in the column

```{r warning=FALSE}
meta = read.table("DOE.Q2FWMojaveBacMetadata.tsv",
                  header=TRUE,row.names=1,
                  sep="\t",stringsAsFactors=FALSE)
```

Check if your metadata file has been import successfully and correctly, the output will show a table of your metadata file (mapping file). *If you do not have header, you might start your first row with # (remove # and reload your mapping file).
```{r}
tail(meta)
```

```{r}
meta = meta[which(meta$Description %in% c("Nicole")),]
```

```{r}
head(meta)
```

Construct sample_data-class using imported metadata
```{r warning=FALSE}
sampleData <- sample_data(meta)
```

###STEP7: Construct Phyloseq object
To construct phyloseq object, otu table, taxonomy table, and sampleData are required. Phylogenetic tree can be included, but it is not necessary for constructing phyloseq object.
Construct Phyloseq object called "Physeq"

```{r warning=FALSE}
physeq = phyloseq(OTU,TAX,sampleData,tree)
```

###STEP8: Check phyloseq object
This should indicate that your physeq is a "phyloseq-class experiment-level object"

```{r}
physeq
```

###STEP9: Remove singletons
Remove any OTUs that present only one time.

```{r}
physeq.prune = prune_taxa(taxa_sums(physeq) > 5, physeq)
```

###STEP10: Plot read counts to check dataset
Check read counts: any samples that have very low reads should be removed.
[Ref](http://evomics.org/wp-content/uploads/2016/01/phyloseq-Lab-01-Answers.html)

```{r}
readcount = data.table(as(sample_data(physeq.prune), "data.frame"),
                 TotalReads = sample_sums(physeq.prune), 
                 keep.rownames = TRUE)
setnames(readcount, "rn", "SampleID")

#For plotting, use command below.
#SeqDepth = ggplot(readcount, aes(TotalReads)) + geom_histogram() + ggtitle("Sequencing Depth")
```

TotalReads of all the samples can be in this table (select only SampleID and TotalReads columns).
In order to check samples with low number of reads, "order()" can be used to sort "TotalReads" column.
In this dataset, N55.Rhizo has very low number of reads, so will will filter this sample out using the next minimum number of reads.
```{r}
readcount = readcount[order(readcount$TotalReads), c("SampleID", "TotalReads")]
```

```{r}
head(readcount)
```

###STEP11: Rarefy OTUs to a minimum number of reads
Rarefy OTUs (remove any samples that has very low number of reads)
If this step work fine, you have successfully imported data to R and completely generate phyloseq object as "physeq.prune.rarefy".
```{r warning=FALSE}
set.seed(711)
physeq.prune.rarefy = rarefy_even_depth(physeq.prune, sample.size = 43108, replace = FALSE, trimOTUs = TRUE)
physeq.prune.rarefy
```

### Remove non-bacterial ASVs

```{r}
Bac.physeq.prune.rarefy = subset_taxa(physeq.prune.rarefy, Kingdom != "Rhizaria")
Bac.physeq.prune.rarefy = subset_taxa(Bac.physeq.prune.rarefy, Kingdom != "Chromista")
Bac.physeq.prune.rarefy = subset_taxa(Bac.physeq.prune.rarefy, Kingdom != "Unassigned")
Bac.physeq.prune.rarefy = subset_taxa(Bac.physeq.prune.rarefy, Kingdom != "D_0__Eukaryota")
Bac.physeq.prune.rarefy
```

###STEP12: Alpha Diversity plot

```{r}
plot_richness(physeq.prune.rarefy, x="Crust_type", color=("Crust_type"), measures=c("Observed")) + 
  geom_boxplot() + theme_bw() + ggtitle("Bacterial Alpha Diversity by Crust_type (Sheephole)") + 
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(axis.text.x = element_text(angle = 90, hjust = 0)) +
  stat_compare_means(method = "anova", label.y = 1580)
```

Comparing by Crust type

```{r}
BacPhylum = as.character(get_taxa_unique(Bac.physeq.prune.rarefy, "Phylum"))
BacPhylum = BacPhylum[complete.cases(BacPhylum)]
BacPhylum
```

```{r}
pdf("./Figures/Bacterial Phylum Alpha (Observed) Diversity by Crust_type .pdf", width = 8, height = 5 )
for (i in BacPhylum) {
  APS = subset_taxa(Bac.physeq.prune.rarefy, Phylum == i)
  APS.plot = plot_richness(APS, x="Crust_type", color =("Crust_type"), measures=c("Observed")) + geom_boxplot() + ggtitle(i) + stat_compare_means(method = "anova") + theme(plot.title = element_text(hjust = 0.5))
  print(APS.plot)
}
dev.off()
```

###STEP13: Calculate ordination 

```{r}
Bac.physeq.prune.rarefy = subset_samples(Bac.physeq.prune.rarefy, Crust_type !="GLC")
```

```{r}
Bac.physeq.prune.rarefy.rf = prune_taxa(taxa_sums(Bac.physeq.prune.rarefy) > 10, Bac.physeq.prune.rarefy)
Bac.physeq.prune.rarefy.rf
```

```{r}
physeq.prune.ord <- ordinate(Bac.physeq.prune.rarefy.rf, "PCoA", "unifrac")
```

```{r}
sample_data(Bac.physeq.prune.rarefy)
```


###STEP14: Beta Diversity Plot

```{r }
plot_ordination(Bac.physeq.prune.rarefy, physeq.prune.ord, type = "samples", color = "Crust_type", shape = "Crust_type", label = "SampleName")  + theme_bw() + ggtitle("Bacterial Beta Diversity (PCoA) by Crust_type (Sheephole)") + theme(plot.title = element_text(hjust = 0.5)) + stat_ellipse(geom = "polygon", alpha = 1/8, aes(fill = Crust_type)) + geom_point(size=4) 
```

```{r}
pdf("./Figures/DOE.Beta_div_Bac_SHP_no_outliers_w_SampleName.pdf", width = 8, height = 5)
plot_ordination(Bac.physeq.prune.rarefy, physeq.prune.ord, type = "samples", color = "Crust_type", shape = "Crust_type", label = "SampleName")  + theme_bw() + ggtitle("Bacterial Beta Diversity (PCoA) by Crust_type (Sheephole)") + theme(plot.title = element_text(hjust = 0.5)) + stat_ellipse(geom = "polygon", alpha = 1/8, aes(fill = Crust_type)) + geom_point(size=4)
dev.off()
```


```{r}
plot_ordination(physeq.prune.rarefy, physeq.prune.ord, type = "samples", color = "Crust_type", shape = "Crust_type")  + theme_bw() + ggtitle("Bacterial Beta Diversity (PCoA) by Crust_type (Sheephole)") + theme(plot.title = element_text(hjust = 0.5)) + geom_point(size=5) + scale_colour_manual(values = c("#000000", "#E69F00", "#56B4E9", "#009E73", "#CC79A7", "#000000", "#E69F00", "#56B4E9", "#009E73", "#CC79A7"))
```


###Plot Taxonomic composition

###Reorder Site and layer
```{r}
sample_data(Bac.physeq.prune.rarefy)$Crust_type = factor(sample_data(Bac.physeq.prune.rarefy)$Crust_type, levels = c("LAC", "CLC", "GLC"))
```

```{r}
Bac.physeq.prune.rarefy.class = tax_glom(Bac.physeq.prune.rarefy, "Class")
Bac.physeq.prune.rarefy.class
```

```{r}
pdf("./Figures/taxcom_Bac_SHP_phylum_abundance).pdf", width = 8, height = 5)
plot_bar(Bac.physeq.prune.rarefy.class, x = "Crust_type", y = "Abundance", fill ="Phylum") + geom_bar(aes(color=Phylum, fill=Phylum), stat="identity", position="stack") + 
  ggtitle("Sheephole Taxonomic Composition (Phylum level) by Crust_type")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
dev.off()
```



```{r fig.height=5, fig.width=8, fig.align="center"}
taxcom_Bac_SHP = ggplot(data = psmelt(Bac.physeq.prune.rarefy), mapping = aes_string(x = "Crust_type" ,y = "Abundance", fill = "Phylum" )) + 
  geom_bar(stat="identity", position="fill") + 
  ggtitle("Sheephole Taxonomic Composition (Phylum level) by Crust_type")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
print(taxcom_Bac_SHP)
```

```{r}
pdf("./Figures/taxcom_Bac_SHP_phylum_relative_abundance_LAC_CLC.pdf", width = 8, height = 5)
taxcom_Bac_SHP
dev.off()
```


###Acidobacteria
```{r}
sample_data(physeq.prune.rarefy)$Crust_type = factor(sample_data(physeq.prune.rarefy)$Crust_type, levels = c("LAC", "CLC"))
```

```{r}
Bac.physeq.prune.rarefy.acidobacteria = subset_taxa(Bac.physeq.prune.rarefy, Phylum == "D_1__Acidobacteria")
Bac.physeq.prune.rarefy.acidobacteria
```
```{r fig.height=5, fig.width=8, fig.align="center"}
taxcom_acidobacteria = ggplot(data = psmelt(Bac.physeq.prune.rarefy.acidobacteria), mapping = aes_string(x = "Crust_type" ,y = "Abundance", fill = "Family" )) + geom_bar(stat="identity", position="fill") + 
  ggtitle("Sheephole Taxonomic Composition (Acidobacteria) by Crust_type")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
print(taxcom_acidobacteria)
```

```{r}
pdf("./Figures/DOE.acidobacteria_taxcom_Bac_SHP_relative_abundance.pdf", width = 8, height = 5)
taxcom_acidobacteria
dev.off()
```

```{r}
Bac.physeq.prune.rarefy.actinobacteria = subset_taxa(Bac.physeq.prune.rarefy, Phylum == "D_1__Actinobacteria")
```

```{r fig.height=5, fig.width=8, fig.align="center"}
taxcom_actinobacteria = ggplot(data = psmelt(Bac.physeq.prune.rarefy.actinobacteria), mapping = aes_string(x = "Crust_type" ,y = "Abundance", fill = "Family" )) + geom_bar(stat="identity", position="fill") + 
  ggtitle("Sheephole Taxonomic Composition (Actinobacteria) by Crust_type")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
print(taxcom_actinobacteria)
```

```{r}
pdf("./Figures/DOE.Actinobacteria_taxcom_Bac_SHP_relative_abundance.pdf", width = 11, height = 7)
taxcom_actinobacteria
dev.off()
```


```{r}
Bac.physeq.prune.rarefy.Cyanobacteria = subset_taxa(Bac.physeq.prune.rarefy, Phylum == "D_1__Cyanobacteria")
```

```{r fig.height=5, fig.width=8, fig.align="center"}
taxcom_Cyanobacteria = ggplot(data = psmelt(Bac.physeq.prune.rarefy.Cyanobacteria), mapping = aes_string(x = "Crust_type" ,y = "Abundance", fill = "Family" )) + geom_bar(stat="identity", position="fill") + 
  ggtitle("Sheephole Taxonomic Composition (Cyanobacteria) by Crust_type")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
print(taxcom_Cyanobacteria)
```

```{r}
pdf("./Figures/DOE.Cyanobacteria_taxcom_Bac_SHP_relative_abundance.pdf", width = 8.5, height = 5.5)
taxcom_Cyanobacteria
dev.off()
```


```{r}
Bac.physeq.prune.rarefy.Firmicutes = subset_taxa(Bac.physeq.prune.rarefy, Phylum == "D_1__Firmicutes")
```

```{r fig.height=5, fig.width=8, fig.align="center"}
taxcom_Firmicutes = ggplot(data = psmelt(Bac.physeq.prune.rarefy.Firmicutes), mapping = aes_string(x = "Crust_type" ,y = "Abundance", fill = "Family" )) + geom_bar(stat="identity", position="fill") + 
  ggtitle("Sheephole Taxonomic Composition (Firmicutes) by Crust_type")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
print(taxcom_Firmicutes)
```

```{r}
pdf("./Figures/DOE.Firmicutes_taxcom_Bac_SHP_relative_abundance.pdf", width = 8, height = 5)
taxcom_Firmicutes
dev.off()
```


###Unique/Endemic

```{r}
Bac.physeq.prune.rarefy
```

```{r}
sample_data(Bac.physeq.prune.rarefy)
```


```{r}
physeq.prune.rarefy.merge.Crust_type <- merge_samples(Bac.physeq.prune.rarefy, "Crust_type_Site")
sample_data(physeq.prune.rarefy.merge.Crust_type)$Crust_type_Site <- factor(sample_names(physeq.prune.rarefy.merge.Crust_type))
physeq.prune.rarefy.merge.Crust_type
```

```{r}
physeq.prune.rarefy.merge.Crust_type.binary = transform_sample_counts(physeq.prune.rarefy.merge.Crust_type, function(abund) 1*(abund>0))
```

```{r}
OTU_SF_merge_crust_type = otu_table(physeq.prune.rarefy.merge.Crust_type.binary)
OTU_SF_merge_crust_type = t(OTU_SF_merge_crust_type)
head(OTU_SF_merge_crust_type)
```

```{r}
OTU_SF_merge_crust_type_endemic = OTU_SF_merge_crust_type[rowSums(OTU_SF_merge_crust_type == 1) == 1, ]
head(OTU_SF_merge_crust_type_endemic)
```

```{r}
OTU_SF_merge_crust_type_endemic_df = data.frame(OTUendemic = row.names(OTU_SF_merge_crust_type_endemic))
dim(OTU_SF_merge_crust_type_endemic_df)
```

```{r warning=FALSE}
otu <- read.table("MojaveBacQ2FWLE.otu_table.fix.txt",header=T,sep="\t")
otus.filtered = otu[otu$OTU.ID %in% OTU_SF_merge_crust_type_endemic_df$OTUendemic,]
otus.filtered.rn = otus.filtered[,-1]
rownames(otus.filtered.rn) = otus.filtered[,1]
otumat.endemic <- as(as.matrix(otus.filtered.rn), "matrix")
OTU.endemic = otu_table(otumat.endemic, taxa_are_rows = TRUE)
```

```{r warning=FALSE}
physeq.endemic = phyloseq(OTU.endemic,TAX,sampleData,tree)
```

```{r}
physeq.endemic
```

```{r}
physeq.endemic.prune = prune_taxa(taxa_sums(physeq.endemic) > 5, physeq.endemic)
```

```{r}
physeq.endemic.prune
```

```{r}
readcount = data.table(as(sample_data(physeq.endemic.prune), "data.frame"),
                 TotalReads = sample_sums(physeq.endemic.prune), 
                 keep.rownames = TRUE)
setnames(readcount, "rn", "SampleID")
readcount = readcount[order(readcount$TotalReads), c("SampleID", "TotalReads")]
head(readcount)
```

```{r warning=FALSE}
set.seed(711)
physeq.endemic.prune.rarefy = rarefy_even_depth(physeq.endemic.prune, sample.size = 4164, replace = FALSE, trimOTUs = TRUE)
physeq.endemic.prune.rarefy
```


```{r}
physeq.endemic.prune.rarefy.merge.Crust_type <- merge_samples(physeq.endemic.prune.rarefy, "Crust_type")
sample_data(physeq.endemic.prune.rarefy.merge.Crust_type)$Crust_type <- factor(sample_names(physeq.endemic.prune.rarefy.merge.Crust_type))
physeq.endemic.prune.rarefy.merge.Crust_type
```

```{r}
physeq.endemic.prune.rarefy = subset_samples(physeq.endemic.prune.rarefy, Crust_type !="GLC")
```

```{r}
physeq.endemic.prune.rarefy
```

```{r}
Heatmap.crusttype.endemic = prune_taxa(names(sort(taxa_sums(physeq.endemic.prune.rarefy),TRUE)[1:50]), physeq.endemic.prune.rarefy)
```


```{r}
badTaxa = c("8a30168b28e042a16f4d6f53b343832b")
goodTaxa <- setdiff(taxa_names(Heatmap.crusttype.endemic), badTaxa)
Heatmap.crusttype.endemic2 <- prune_taxa(goodTaxa, Heatmap.crusttype.endemic)
Heatmap.crusttype.endemic2
```



```{r}
Heatmap.crusttype.endemic.plot = plot_heatmap(Heatmap.crusttype.endemic2, "PCoA", "unifrac", "Crust_type", "Family", sample.order = "Crust_type")+ ggtitle("Bacterial endemic species heatmap (top 50 ASVs)") + theme(plot.title = element_text(hjust = 0.5))
print(Heatmap.crusttype.endemic.plot)

ggsave("./Figures/Bacterial_endemic_species_heatmap_top50_class.png", plot = Heatmap.crusttype.endemic.plot, width = 12, height = 8)
```

### Indicator Species (Andrew)

Convert phyloseq OTU table to reflect species

```{r}
Bac.physeq.prune.rarefy
```

Glomerate at Species level (However, not all of species will be unique)
See for ref: https://github.com/joey711/phyloseq/issues/927

```{r}
phyloGlom = tax_glom(Bac.physeq.prune.rarefy,taxrank = "Species")
```

```{r}
phyloGlom
```
Instead of using unique Species name, OTU.ID will be used instead becasue they are unique
```{r}
glomTax = tax_table(phyloGlom)[,"OTU.ID"]
```

Combine OTU table and Tax table
```{r}
glomOTU = otu_table(phyloGlom)
glomTable = merge(glomOTU,glomTax,by=0,all=TRUE)
head(glomTable)
```

Use OTU.ID as rowname and remove prior rowname and OTU.ID
```{r}
rownames(glomTable) = glomTable[,"OTU.ID"]
glomTable$Row.names = NULL
glomTable$OTU.ID = NULL
```

```{r}
head(glomTable)
```


Transpose Rows and columns for desired labdsv format
```{r}
ASV.SHP = t(glomTable)
```

Remove columns with 0 species counts

```{r}
ASV.SHP.no0 = ASV.SHP[, colSums(ASV.SHP != 0) > 0]
```

Subset sampleData phyloseq object for the desired variable (e.g. treatment, depth, crust type, etc.)

```{r}
Crust_type.SHP = subset(sampleData,select = "Crust_type")
```

```{r}
class(sampleData$Crust_type_Site)
```


Make the varible numeric

```{r}
sampleData$Crust_type
```

```{r}
Crust_type.SHP$Crust_type = as.numeric(as.factor(Crust_type.SHP$Crust_type))
Crust_type.SHP$Crust_type
```


Run indval code from lab.dsv 

```{r}
LAC.Crust_type.SHP = indval(ASV.SHP.no0,Crust_type.SHP$Crust_type)
```

```{r}
summary(LAC.Crust_type.SHP, p = 0.01, type = "short")
```

```{r}
summary(LAC.Crust_type.SHP, p = 0.05, type = "long")
```

Get indval value
```{r}
LAC.Crust_type.SHP$indval
```

```{r}
LAC.Crust_type.SHP$indcls
```

###Get Significant Indicators (203 ASVs)

```{r}
gr <- LAC.Crust_type.SHP$maxcls[LAC.Crust_type.SHP$pval<=0.01]
iv <- LAC.Crust_type.SHP$indcls[LAC.Crust_type.SHP$pval<=0.01]
pv <- LAC.Crust_type.SHP$pval[LAC.Crust_type.SHP$pval<=0.01]
fr <- apply(ASV.SHP.no0[,-1]>0, 2, sum)[LAC.Crust_type.SHP$pval<=0.01]
indvalsummary <- data.frame(group=gr, indval=iv, pvalue=pv, freq=fr)
indvalsummary <- indvalsummary[order(indvalsummary$group, -indvalsummary$indval),]
```

```{r}
indvalsummary
```

remove prepend X from rownames function

```{r}
rowClean <- function(x){ rownames(x) <- gsub("X", "", rownames(x)); x }
```


```{r}
indvalsummary = rowClean(indvalsummary)
indvalsummary
```


```{r}
SHP.Crust_type.indval = as.data.frame(LAC.Crust_type.SHP$indval)
SHP.Crust_type.indval = rowClean(SHP.Crust_type.indval)
SHP.Crust_type.indval
```

Match rowname of indval table to summary table and take only 203 matches indicator species

```{r}
SHP.Crust_type.indval_filtered = SHP.Crust_type.indval[row.names(SHP.Crust_type.indval) %in% row.names(indvalsummary),]
SHP.Crust_type.indval_filtered
```

Rename columns
```{r}
names(SHP.Crust_type.indval_filtered)[names(SHP.Crust_type.indval_filtered) == "1"] <- "CLC"
names(SHP.Crust_type.indval_filtered)[names(SHP.Crust_type.indval_filtered) == "2"] <- "GLC"
names(SHP.Crust_type.indval_filtered)[names(SHP.Crust_type.indval_filtered) == "3"] <- "LAC"
head(SHP.Crust_type.indval_filtered)
```

Add otu table from filtered indval table
```{r}
indic.otumat <- as(as.matrix(SHP.Crust_type.indval_filtered), "matrix")
indic.OTU = otu_table(indic.otumat, taxa_are_rows = TRUE)
```

```{r}
head(indic.OTU)
```

add metadata table
```{r warning=FALSE}
indic.meta = read.table("DOE.indic.meta.tsv",
                  header=TRUE,row.names=1,
                  sep="\t",stringsAsFactors=FALSE)
```

```{r}
indic.meta
```


```{r warning=FALSE}
indic.sampleData <- sample_data(indic.meta)
```

```{r warning=FALSE}
indic.physeq = phyloseq(indic.OTU,TAX,indic.sampleData)
```

```{r}
indic.physeq
```



```{r}
#Heatmap.indic.physeq = prune_taxa(names(sort(taxa_sums(indic.physeq),TRUE)[1:203]),indic.physeq)
```

```{r}
indic.physeq.noGLC = subset_samples(indic.physeq, Crust_type !="GLC")
```

```{r}
indic.physeq.noGLC
```

```{r}
sample_data(indic.physeq)$Crust_type = factor(sample_data(indic.physeq)$Crust_type, levels = c("LAC", "CLC", "GLC"))
```


```{r}
indic.physeq.TAX = tax_table(indic.physeq)
indic.physeq.OTU.Tax.tab = merge(indvalsummary,indic.physeq.TAX,by=0,all=TRUE)
indic.physeq.OTU.Tax.tab
```

```{r}
write.csv(indic.physeq.OTU.Tax.tab, file="indvalsummary.crusttype.SHP.csv")
```

```{r}
Heatmap.indic.physeq = prune_taxa(names(sort(taxa_sums(indic.physeq),TRUE)[1:65]),indic.physeq)
```

```{r}
Heatmap.indic.physeq.plot = plot_heatmap(Heatmap.indic.physeq, "PCoA", "bray", sample.label = NULL ,"Genus", sample.order = c("LAC", "CLC", "GLC"))+ ggtitle("Bacterial indicator species heatmap (P < 0.01)") + theme(plot.title = element_text(hjust = 0.5))
print(Heatmap.indic.physeq.plot)
```

```{r}
ggsave("./Figures/Heatmap_indicator_species_Genuslevel_0.01P.png", plot = Heatmap.indic.physeq.plot, width = 12, height = 8)
```





